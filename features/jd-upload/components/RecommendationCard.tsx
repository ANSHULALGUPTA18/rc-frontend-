              ...
              <div className="space-y-5">
                ...
                {rec.submissionStatus === "approved" ? (
                  <div className="rounded-lg border border-green-200 bg-green-50 p-4 text-center">
                    ...
                    <p className="mt-1 text-xs text-green-600">This pricing recommendation has been approved by admin.</p>
                  </div>
                ) : rec.submissionStatus === "rejected" ? (
                  <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-center">
                    <p className="text-sm font-semibold text-red-700">Rejected</p>
                    <p className="mt-1 text-xs text-red-600">This pricing recommendation was rejected by admin.</p>
                  </div>
                ) : rec.submissionStatus === "pending_approval" || approvalState === "submitted" ? (
                  <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-center">
                    <p className="text-sm font-semibold text-amber-700">Pending Admin Approval</p>
                    <p className="mt-1 text-xs text-amber-600">Waiting for admin to review this recommendation.</p>
                  </div>
                ) : (
                  <div className="rounded-lg border border-line bg-surface-muted p-4 space-y-3">
                    {!showNotesInput ? (
                      <div className="flex items-center justify-between">
                        <p className="text-sm text-ink-muted">Ready to submit this recommendation?</p>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-end gap-2">
                          <Button
                            variant="secondary"
                            size="sm"
                            onClick={() => { setShowNotesInput(false); setApprovalNotes(""); }}
                          >
                            Cancel
                          </Button>
                          <Button
