...
export function DashboardView(): React.ReactElement {
  ...
  const {
    ...
    error: kpiError,
  } = useQuery({ queryKey: ["kpi"], queryFn: () => getKpiStats(msal) });
  const {
    data: approvals = [],
    isLoading: approvalsLoading,
    error: approvalsError,
    refetch: refetchApprovals,
  } = useQuery({ queryKey: ["approvals"], queryFn: () => getApprovals(msal) });
  const {
    data: reports = [],
          ...
          <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
            ...
            {kpi && (
              <>
                ...
                />
                <KpiCard label="Avg Margin" value={kpi.avgMargin} subtext={<OptimalBadge />} />
              </>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_300px]">
              ...
              <div className="flex items-center justify-between border-t border-line px-6 py-3">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setPage((p) => Math.min(Math.ceil(approvals.length / PAGE_SIZE), p + 1))
                    }
                    className="flex h-7 w-7 items-center justify-center rounded-md border border-line text-ink-subtle hover:bg-surface-muted disabled:opacity-30 disabled:cursor-not-allowed"
                  >
                    <svg
            ...
            <div className="rounded-card border border-line bg-surface shadow-card">
              ...
              {!reportsLoading && !reportsError && reports.length === 0 && (
                <div className="px-5 py-10 text-center text-sm text-ink-muted">No reports yet.</div>
              )}
              {!reportsLoading && !reportsError && reports.length > 0 && (
